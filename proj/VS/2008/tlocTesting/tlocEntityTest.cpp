...
namespace TestingEntity
{
  ...
  TEST_CASE("Core/component_system/entity/entity", "")
  {
    ...
    CHECK(e->GetID() == 0);
    CHECK(e->GetComponentsList().size() == (tl_size)components::count);
    ...
    CHECK(e->HasComponent(components::mesh) == false);
    ...
    CHECK(e->HasComponent(components::transform) == true);
    CHECK(e->HasComponent(components::mesh) == true);
    ...
    clist = e->GetComponents(components::mesh);
    REQUIRE(clist.size() == 1);
    CHECK(clist[0] == &c2);
  }
};